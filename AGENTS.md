...
Async tests must wait on the observable condition, never a fixed sleep — a hand-rolled `Task.sleep` poll is the documented flake (`docs/solutions/test-failures/async-fake-sleep-race-flaky-tests.md`; one such race failed 6 CI runs across 4 PRs in a day). Use the poll-based `Whistle_waitUntil` helper (`apps/macos/WhistleTests/CaptureViewModelTests.swift:206`) or a continuation-based waiter on the fake (e.g. `waitForStartTaskCallCount`). Prove a flake fix by looping the single test 20+ times with `xcodebuild test-without-building` — but prefer running that loop in CI (ephemeral runner, no login keychain), not against your own machine (next paragraph). The strongest flake fix is a deterministic seam that makes the race structurally impossible, so a single green run suffices — reach for that before brute-force repetition.

App-hosted test runs and the login keychain: `xcodebuild test` launches the full `Whistle.app` as the test host, so `WhistleApp`'s composition root (`applicationDidFinishLaunching`) runs on every launch. Because each build is ad-hoc signed with a shifting signature, any launch-time login-keychain access re-prompts — and a 20×-in-a-loop local run means 20 macOS keychain dialogs. The `WHISTLE_TESTING=1` env var in `project.yml`'s test scheme prevents this: `WhistleApp.isRunningUnderTest` then swaps in `InMemoryAuthBreadcrumbStore` + `DevSignInAuthProvider` instead of the Keychain-backed breadcrumb store and `Auth0AuthProvider` (whose `CredentialsManager` reads the bundle-id keychain). Don't remove that guard, and don't add new launch-time keychain/network side effects to the composition root without gating them the same way — the app-host launch is untested (composition roots are MANUAL-QA's job), so a real side effect there only surfaces as a per-run prompt.
