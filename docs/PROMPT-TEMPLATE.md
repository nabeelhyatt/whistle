...
Workspace names are generated **server-side in the pipeline** (TECH-SPEC §6), not by the app: a Claude Haiku call (via OpenRouter) produces a 3–5 word, noun-first title from the transcript/notes/project (`titleGenerator.ts`); the pipeline appends ` #<clientId prefix>` — e.g. `Capture panel redesign #a1b2c3`. If title generation is unavailable (no `OPENROUTER_API_KEY`, a non-200 response, or a timeout) it falls back to `<first ~6 meaningful words of notes-or-transcript> #<clientId prefix>`, and further to `Screenshot capture <date> #<clientId prefix>` for screenshot-only captures. The `#<clientId>` tag is always present and load-bearing (orphan-workspace adoption on retry) — custom templates may change the prompt freely, but naming is not template-controlled.
