...
## Endpoints available but unused in v1

`GET /v0/projects/{id}`, `GET /v0/workspaces/{id}`, `POST /v0/workspaces/{id}/rename`, `POST /v0/workspaces/{id}/archive`, `POST /v0/sessions` (extra sessions in an existing workspace; body requires `{ workspaceId, agent }`), `GET /v0/sessions/{id}`, `POST /v0/sessions/{id}/rename`, `GET /v0/messages/{id}`, `POST /v0/sessions/{id}/cancel`.

Potential later uses: `archive` for a history "clean up" action; `cancel` for aborting a runaway first pass; `rename` to sync workspace names if capture titles become editable.
...
5. Whether the workspace agent has outbound network access to fetch the screenshot URL (expected yes — agents can curl). If not, screenshots become history-only and the prompt's screenshot block is dropped.
