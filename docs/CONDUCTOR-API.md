...
4. **Unresolved — blocked** (workspace init failure, above; no agent ever replied, so no real `content` to capture). The only shape observed live is the empty list: `GET .../messages` → `{ "data": [] }` — notably it stays empty even after a successful 201 `queued` send, so a *sent* message is not necessarily *listed* until the workspace processes it. Two consequences: (a) the real agent-message fixture (`packages/backend/convex/__tests__/fixtures/agent-message.json`) still needs capturing — fold it into #5's pass-2 re-run; (b) TECH-SPEC §6 step 4a's guard ("skip if our messageId is listed") can legitimately see an empty list after a send that actually succeeded, which is safe only because a duplicate send hard-fails loudly rather than double-prompting (#6). Keep parsing `content` defensively.
