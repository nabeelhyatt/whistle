...
- **`precious-loris-637` (production) — this is what ships.** `apps/macos/Config/Convex.xcconfig` points the app here (since 1.0.20), and `backend-deploy.yml` *attempts* `npx convex deploy` against it on every push to main touching `packages/backend/**` — attempts, because the deploy step is gated on the `CONVEX_DEPLOY_KEY` secret and **silently skips while still reporting the workflow green** when it's absent. A green run is not proof of deployment; check that the `Deploy to Convex` step's conclusion is `success` rather than `skipped` (`SECRETS.md` §4).
