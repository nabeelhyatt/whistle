...
- **`precious-loris-637` (production) — this is what ships.** `apps/macos/Config/Convex.xcconfig` points the app here (since 1.0.20), and `backend-deploy.yml` deploys it via `npx convex deploy` on every push to main touching `packages/backend/**`.
- **`grandiose-alpaca-243` (development)** — local work only, pushed to by `npx convex dev --once`. Never ship a build pointing here.

Local work targets dev non-interactively via `.env.local` / `CONVEX_DEPLOYMENT` plus `npx convex dev --once`, and must never invoke interactive `convex dev` project configuration. Both deployments need the same env vars set (`AUTH0_DOMAIN`, `AUTH0_AUDIENCE`, `OPENROUTER_API_KEY` — see `SECRETS.md` §7); a var set on one is *not* visible to the other, and a missing `OPENROUTER_API_KEY` degrades silently to the heuristic workspace name rather than failing.
