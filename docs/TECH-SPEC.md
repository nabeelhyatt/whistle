...
**Convex.** The Convex CLI is already logged in on this machine, and a project already exists: team `nabeelo`, project `whistle`. It has two deployments, and which one you're touching matters:

- **`precious-loris-637` (production) — this is what ships.** `apps/macos/Config/Convex.xcconfig` points **Release** builds here (since 1.0.20; Debug builds resolve the dev deployment below via a `CONVEX_URL[config=Debug]` override in the same file — Debug is never shipped, so "never ship a build pointing at dev" holds). `backend-deploy.yml` runs `npx convex deploy` against it on every push to main touching `packages/backend/**`, and **fails the run red at its `Require CONVEX_DEPLOY_KEY` step** if the secret is missing — it no longer skips-and-passes (that green-while-undeployed gate is how PR #38 sat unshipped for a day; the runbook checks in `SECRETS.md` §4 are now confirmatory, not the primary signal). Note the workflow triggers post-merge, so a red run notifies rather than blocks. One-time history: prod was seeded from dev on 2026-08-05 via `npx convex export --include-file-storage` -> `npx convex import --prod --replace-all`, verified byte-identical across all five tables.
- **`grandiose-alpaca-243` (development)** — local work only, pushed to by `npx convex dev --once`. Never ship a build pointing here.

Local work targets dev non-interactively via `.env.local` / `CONVEX_DEPLOYMENT` plus `npx convex dev --once`, and must never invoke interactive `convex dev` project configuration. Both deployments need the same env vars set — `AUTH0_DOMAIN` / `AUTH0_AUDIENCE` (`SECRETS.md` §6) and `OPENROUTER_API_KEY` (§7); a var set on one is *not* visible to the other, and a missing `OPENROUTER_API_KEY` degrades silently to the heuristic workspace name rather than failing.

**Why the split is called out this loudly:** through 1.0.19 the app shipped pointing at the *dev* deployment, and `npx convex deploy` can only target a prod or preview deployment — never a dev one. So CI structurally could not deploy the backend users were actually hitting, while `backend-deploy.yml` still reported green (its deploy step was additionally skipped for a missing `CONVEX_DEPLOY_KEY`). PR #38's workspace-title change sat live-in-main-but-undeployed for a day as a result. If you ever repoint the app at a dev deployment, you reintroduce exactly that class of silent staleness.
