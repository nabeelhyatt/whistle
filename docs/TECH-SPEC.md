...
**Authenticated subscription lifecycle.** `HistoryViewModel` keeps its local pending-draft observation active regardless of authentication, while `captures.listRecent` and the app-wide `ProjectsSyncCoordinator` are enabled only for `.signedIn`. One MainActor coordinator consumes distinct `AuthController.state` values and applies History/Projects transitions synchronously, so rapid reauthentication cannot reorder discarded actor tasks. Both server streams use the shared `AuthenticatedSubscription` supervisor: unexpected terminal completion retries with capped cancellation-aware backoff, repeated enable is idempotent, and generation contexts fence values that cross an actor boundary before being applied. Settings does not own a second `projects.list` stream; it observes the coordinator-persisted `CaptureStore.projectsUpdates()` snapshot. A sign-in beginning from `.reauthRequired` detaches the stale Convex auth bridge before interactive login and `users.ensure`.
...
- **CI (GitHub Actions)**: three workflows — `ci.yml` (macOS runner: SwiftLint, `xcodebuild test`, on every PR), `release.yml` (builds/signs the DMG, attempts notarization per the env-gating above, generates a Sparkle appcast entry, uploads to GitHub Releases), `backend-deploy.yml` (`npx convex deploy` for `packages/backend`, Next.js deploy for `apps/web`). `apps/web` serves the appcast XML + download page — **this is not the Next.js you know**; read `node_modules/next/dist/docs/` before touching `apps/web` code, per `AGENTS.md`, before making any changes here (including the appcast route).
