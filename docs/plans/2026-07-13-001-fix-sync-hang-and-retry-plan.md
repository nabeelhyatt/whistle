...
1. **Add a timeout wrapper to `authedMutation`/`authedAction`, mirroring the existing `firstValue(from:timeout:)` pattern** (`ConvexService.swift`) rather than inventing a new mechanism. That function already races a `withThrowingTaskGroup` against `Task.sleep(for:)` for one-shot queries and is unit-tested (`ConvexOneShotQueryTests`) — extract the race-and-cancel shape into a small generic helper (e.g. `withTimeout(_:operation:)`) both `firstValue` and the mutation/action wrappers can call, instead of duplicating the task-group boilerplate three times.
