...
// a single Claude Haiku call, routed through OpenRouter. Mirrors
// conductorClient.ts's raw-`fetch` idiom (no SDK dependency in the Convex
// default runtime) and its "never die silently" spirit — but inverted: this
// helper is purely cosmetic (it only improves `buildWorkspaceName`'s output
// in pipeline.ts), so it must NEVER throw and NEVER stall the pipeline.
// Every failure mode (missing key, non-200, network error, timeout)
// collapses to `null`, and callers fall back to the heuristic name.
...
/**
 * Calls Claude Haiku (via OpenRouter) to produce a short workspace title for
 * a captured idea. Never throws: returns `null` on a missing API key, a
 * non-200 response, a network error, or a timeout, so `pipeline.ts` can
 * always fall back to the heuristic name in `buildWorkspaceName` without the
 * pipeline ever failing or stalling because of this call.
 */
export async function generateWorkspaceTitle(args: {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `titleGenerator: OpenRouter API error — status=${res.status}`,
      );
      return null;
    }
